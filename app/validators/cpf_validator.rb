class CpfValidator < ActiveModel::EachValidator
	...
	def validate_each(record, attribute, value)
		...
		record.errors.add(
			attribute,
			:invalid_cpf,
			message: options[:message] || I18n.t('cpf_is_not_valid'),
			value: value
		)
	end
  ...
  SIZE_VALIDATION = /^[0-9]{11}$/
	...
	def cpf_valid?(cpf)
		...
		return if VALUE_INVALID.include?(cpf)
	...
	def first_digit_valid?(cpf_numbers)
		...
		multiplied = first_digit_valid?.map.with_index do |number, index|
			number * (10 - index)
		end
	...
	def second_digit_valid?(cpf_numbers)
		...
		snd_verified_digit = 11 - mod > 9 ? 0 :mod
		snd_verified_digit == cpf_numbers[11]
	end
end